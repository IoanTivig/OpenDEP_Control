...
# Local imports #
from src.classes.dsrl_camera_control import *
from src.funcs.general_use_functions import *
...
class LiveCaptureWorker(QObject):
    ...
    def multi_capture_process(self):
        ...
        # Create a logarithmic list with all frequencies
        ...
        for i in range(len(frequency_list)):
            ...
            if (
                self.live_capture_UI.pyqt5_dynamic_odsc_checkbox_use_generator.isChecked()
            ):
                self.live_capture_UI.generator.set_frequency(frequency_list[i])

            # Update all UI labels for each capture
            current_frequency = frequency_text_format(frequency_list[i])
            if i + 1 < len(frequency_list):
                next_frequency = frequency_text_format(frequency_list[i + 1])
            else:
                next_frequency = "N/A"
            ...
            if (
                ...
                filename = (
                    "OpenDEP_" + str(frequency_int_format(frequency_list[i], 2)) + "Hz"
                )
